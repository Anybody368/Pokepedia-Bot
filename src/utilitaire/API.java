...
public class API {
	...
	private static String bulbaApiURL = "https://bulbapedia.bulbagarden.net/w/api.php";
//	private static String apiURL = "http://localhost/wiki/api.php";
	...
	public static Document post(Hashtable<String, String> parameters, Wiki from) {
		return post(parameters, null, from);
	}
	...
	public static Document post(Hashtable<String, String> parameters, File file, Wiki from) {
		...
		try {
			...
			URL url = switch (from) {
				case POKEPEDIA -> new URL(apiURL);
				case BULBAPEDIA -> new URL(bulbaApiURL);
			};
